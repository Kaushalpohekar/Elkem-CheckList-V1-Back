...
async function login(req, res) {
    ...
    const query = `SELECT * FROM elkem.users WHERE username = $1 OR personal_email = $2`;
...
async function register(req, res) {
    ...
    try {
        ...
        const AddOrganizationQuery = `INSERT INTO elkem.organizations (organization_id, "name", address, created_at) VALUES($1, $2, $3, CURRENT_TIMESTAMP);`;
        await client.query(AddOrganizationQuery, [organization_id, CompanyName, CompanyAddress]);

        const AddPlantQuery = `INSERT INTO elkem.plants (plant_id, organization_id, "name", "location", created_at) VALUES($1, $2, $3, $4, CURRENT_TIMESTAMP);`;
        await client.query(AddPlantQuery, [plant_id, organization_id, CompanyName, CompanyAddress]);

        const AddDepartmentQuery = `INSERT INTO elkem.departments (department_id, plant_id, "name", created_at) VALUES($1, $2, $3, CURRENT_TIMESTAMP);`;
        await client.query(AddDepartmentQuery, [department_id, plant_id, department_name]);

        const AdminUUIDQuery = `SELECT role_id FROM elkem.roles WHERE name = $1;`;
        ...
        const CheckUserExistQuery = `SELECT * FROM elkem.users WHERE personal_email = $1 OR company_email = $2;`;
        ...
        const InsertUserQuery = `INSERT INTO elkem.users (user_id, username, personal_email, password_hash, first_name, last_name, role_id, organization_id, department_id, created_at, company_email, verified, block, contact_no) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP, $10, false, false, $11);`;
...
async function forgotPassword(req, res) {
    ...
    try {
        const query = 'SELECT * FROM elkem.users WHERE personal_email = $1';
        ...
        const insertQuery = 'INSERT INTO elkem.reset_tokens (user_id, token) VALUES ($1, $2)';
...
async function resendResetToken(req, res) {
    ...
    try {
        const checkUserQuery = 'SELECT * FROM elkem.users WHERE personal_email = $1';
        ...
        const updateQuery = 'UPDATE elkem.elkem_reset_tokens SET token = $1 WHERE user_id = $2';
...
async function resetPassword(req, res) {
    ...
    try {
        const query = 'SELECT * FROM elkem.reset_tokens WHERE token = $1';
        ...
        const updateQuery = 'UPDATE elkem.users SET password = $1 WHERE user_id = $2';
        await db.query(updateQuery, [hashedPassword, userId]);

        const deleteQuery = 'DELETE FROM elkem.elkem_reset_tokens WHERE token = $1';
...
async function getUserDetails(req, res) {
    ...
    try {
        ...
        const fetchUserQuery = 'SELECT * FROM elkem.users WHERE user_id = $1';
        ...
        const fetchRoleQuery = 'SELECT * FROM elkem.roles WHERE role_id = $1'; // Assuming companies table and correct column names
...
async function block(req, res) {
    ...
    try {
        const checkQuery = 'SELECT block FROM elkem.users WHERE user_id = $1';
...
async function getAllTokens(req, res) {
    ...
    if (token === matchToken) {
        try {
            const query = 'SELECT * FROM elkem.reset_tokens';
...
async function updateUser(req, res) {
  ...
  const checkUserExistsQuery = 'SELECT 1 FROM elkem.users WHERE user_id = $1';
  const updateQuery = `
    UPDATE elkem.users
    SET first_name = $1,
        last_name = $2
...
async function updatePassword(req, res) {
  ...
  try {
    ...
    const checkUserQuery = `SELECT * FROM elkem.users WHERE user_id = $1`;
    ...
    const updateQuery = `UPDATE elkem.users SET password_hash = $1 WHERE user_id = $2`;
...
async function insertOrUpdateUserProfilePhoto(req, res) {
    ...
    try {
        ...
        const userProfilePhotoCheckQuery = 'SELECT 1 FROM elkem.userprofilepictures WHERE user_id = $1';
        ...
        if (userProfilePhotoResult.rowCount > 0) {
            const updatePhotoQuery = `
                UPDATE elkem.userprofilepictures
            ...
            const insertPhotoQuery = `
                INSERT INTO elkem.userprofilepictures (user_id, photo_name, photo_path)
                VALUES ($1, $2, $3)
            `;
...
async function getProfilePicture(req, res) {
    ...
    try {
        ...
        const query = `SELECT photo_path, photo_name as profile_name FROM elkem.userprofilepictures WHERE user_id = $1`;
